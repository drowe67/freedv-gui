...
void opt_help() {
  ...
  fprintf(stderr,
          "  --mode    modeName    Predefined mode 700D|700E|2020|2020B|datac0 "
          "... etc\n");
  fprintf(stderr,
          "  --nc      [17..62]    Number of Carriers (17 default, 62 max)\n");
  fprintf(stderr,
          "  --ns       symbols    One pilot every ns symbols (8 default)\n");
  fprintf(stderr,
          "  --tcp        Nsecs    Cyclic Prefix Duration (.002 default)\n");
  fprintf(stderr, "  --ts         Nsecs    Symbol Duration (.018 default)\n");
  fprintf(stderr,
          "  --testframes Nsecs    Transmit test frames for Nsec (--testframes "
          "NpacketsPerBurst in burst mode)\n");
  fprintf(stderr,
          "  --tx_freq     freq    Set an optional modulation TX centre "
          "frequency (1500.0 default)\n");
  fprintf(stderr,
          "  --rx_freq     freq    Set an optional modulation RX centre "
          "frequency (1500.0 default)\n\n");
  fprintf(
      stderr,
      "  --verbose  [1|2|3]    Verbose output level to stderr (default off)\n");
  fprintf(
      stderr,
      "  --txbpf               Transmit band pass filter on (default off)\n");
  fprintf(stderr, "  --clip                Transmit clipper (default off)\n");
  fprintf(stderr,
          "  --text                Include a standard text message boolean "
          "(default off)\n");
  fprintf(stderr,
          "  -i --ldpc    [1|2]    Run LDPC decoder (1 -> (224,112) 700D code, "
          "2 -> (504,396) 2020 code).\n"
          "                        In testframe mode raw and coded errors will "
          "be counted.\n");
  fprintf(
      stderr,
      "  --bursts   nBursts    Burst mode: Send nBursts of testframes each\n");
  fprintf(stderr, "\n");
  exit(-1);
}
...
int main(int argc, char *argv[]) {
  ...
  struct optparse_long longopts[] = {{"in", 'a', OPTPARSE_REQUIRED},
  ...
  /* Get a copy of the completed modem config (ofdm_create() fills in more
   * parameters) */
  ...
  if (verbose) {
    ...
    fprintf(
        stderr,
        "Ndatabitsperpacket: %d Npayloadbitsperpacket: %d Nsamperpacket: %d\n",
  ...
  if (testframes) {
    ...
    if (verbose) fprintf(stderr, "Npackets: %d\n", Npackets);
  }
  ...
  for (int b = 0; b < Nbursts; b++) {
    ...
    while (fread(data_bits, sizeof(uint8_t), Ndatabitsperpacket, fin) ==
          ...
          if (use_text) {
            ...
            for (k = 0; k < nspare; k++) {
              ...
              if (nvaricode_bits == 0) {
                ...
                nvaricode_bits = varicode_encode(tx_varicode_bits, s,
                                                 VARICODE_MAX_BITS, 1, 1);
      ...
      if (testframes && (packet >= Npackets)) break;
    }
