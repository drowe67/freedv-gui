...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    fprintf(stderr, "\n");
    fprintf(stderr,
            "usage: %s InputOneBytePerBit OutputFile [--sd] [--code CodeName] "
            "[--testframes Nframes] [--unused numUnusedDataBits]\n",
            argv[0]);
    fprintf(stderr, "\n");
  ...
  while (fread(ibits, sizeof(char), data_bits_per_frame, fin) ==
    ...
    if (sd) {
      ...
      for (i = 0; i < parity_bits_per_frame; i++)
        sdout[i + data_bits_per_frame - unused_data_bits] =
            1.0 - 2.0 * pbits[i];
      written += fwrite(
          sdout, sizeof(float),
          data_bits_per_frame - unused_data_bits + parity_bits_per_frame, fout);
    } else {
