...
int AudioOptsDialog::setTextCtrlIfDevNumValid(wxTextCtrl *textCtrl, wxListCtrl *listCtrl, int devNum)
{
    ...
    for(i=0; i<listCtrl->GetItemCount()-1; i++) {
        ...
        if (aDevNum == devNum) {
            ...
            fprintf(stderr,"setting focus of %d\n", i);
...
int AudioOptsDialog::ExchangeData(int inout)
{
    if(inout == EXCHANGE_DATA_IN)
    {
        ...
        fprintf(stderr,"EXCHANGE_DATA_IN:\n");
        fprintf(stderr,"  g_nSoundCards: %d\n", g_nSoundCards);
        fprintf(stderr,"  g_soundCard1InDeviceNum: %d\n", g_soundCard1InDeviceNum);
        fprintf(stderr,"  g_soundCard1OutDeviceNum: %d\n", g_soundCard1OutDeviceNum);
        fprintf(stderr,"  g_soundCard1SampleRate: %d\n", g_soundCard1SampleRate);
        fprintf(stderr,"  g_soundCard2InDeviceNum: %d\n", g_soundCard2InDeviceNum);
        fprintf(stderr,"  g_soundCard2OutDeviceNum: %d\n", g_soundCard2OutDeviceNum);
        fprintf(stderr,"  g_soundCard2SampleRate: %d\n", g_soundCard2SampleRate);
    ...
    if(inout == EXCHANGE_DATA_OUT)
    {
        ...
        fprintf(stderr,"  valid_one_card_config: %d  valid_two_card_config: %d\n", valid_one_card_config, valid_two_card_config);
...
int AudioOptsDialog:: buildListOfSupportedSampleRates(wxComboBox *cbSampleRate, int devNum, int in_out)
{
    ...
    if (deviceInfo == NULL) {
        fprintf(stderr,"Pa_GetDeviceInfo(%d) failed!\n", devNum);
        cbSampleRate->Clear();
        return 0;
    }
    ...
    {      
        ...
        if( err == paFormatIsSupported ) {
            ...
            fprintf(stderr,"%i ", (int)standardSampleRates[i]);
            numSampleRates++;
        }
    }
    fprintf(stderr,"\n");
...
void AudioOptsDialog::plotDeviceInputForAFewSecs(int devNum, PlotScalar *plotScalar) {
    ...
    fprintf(stderr,"inputChannels: %d nBufs %d\n", inputChannels, nBufs);
...
void AudioOptsDialog::OnOkAudioParameters(wxCommandEvent& event)
{
    ...
    fprintf(stderr,"status: %d m_isPaInitialized: %d\n", status, m_isPaInitialized);
    if (status == 0) {
        if(m_isPaInitialized)
        {
            if((pa_err = Pa_Terminate()) == paNoError)
