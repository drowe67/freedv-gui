...
if [ "$FREEDV_TEST" == "txrx" ]; then 
    ...
    sox $(pwd)/rade_src/wav/all.wav $(pwd)/rade_src/wav/all.wav -r 48000 $(pwd)/tx_in.wav

    # Play input audio
    if [ "$OPERATING_SYSTEM" == "Linux" ]; then
	paplay -d "$FREEDV_MICROPHONE_TO_COMPUTER_DEVICE" $(pwd)/tx_in.wav &
    else
        sox -q --buffer 32768 $(pwd)/tx_in.wav -t $SOX_DRIVER "$FREEDV_MICROPHONE_TO_COMPUTER_DEVICE" &
    fi
    PLAY_PID=$!
    ...
    if [ "$OPERATING_SYSTEM" == "Linux" ]; then
        ...
        sox -q --buffer 32768 $FREEDV_RX_FILE -t $SOX_DRIVER "$FREEDV_RADIO_TO_COMPUTER_DEVICE" &
    fi
    PLAY_PID=$!
