...
if [ $FREEDV_EXIT_CODE -eq 0 ]; then
    ...
    if [ "$2" == "mpp" ]; then
        ...
        sox $(pwd)/test.wav -t raw -r 8000 -c 1 -e signed-integer -b 16 - | $(pwd)/codec2/build_linux/src/ch - - --No -21 | sox -t raw -r 8000 -c 1 -e signed-integer -b 16 - -t wav $(pwd)/testwithnoise.wav
    fi
